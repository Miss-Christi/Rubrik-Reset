...
const AdminDashboard = () => {
    ...
    // Modal states for creating Products and Challenges
    ...
    const standardProducts = products.filter(p => p.category !== 'Challenge');
    const challengeProducts = products.filter(p => p.category === 'Challenge');
                                    ...
                                    <table className="w-full text-left whitespace-nowrap">
                                        ...
                                        <tbody className={`divide-y ${isDark ? "divide-slate-700" : "divide-slate-100"}`}>
                                            {challengeProducts.map(product => (
                                                <tr key={product._id} className={`text-sm ${isDark ? "hover:bg-slate-800" : "hover:bg-slate-50/50"} transition-colors`}>
                                                    <td className={`p-4 font-bold ${textPrimary}`}>{product.title}</td>
                                                    <td className={`p-4 text-xs font-bold ${textSecondary}`}>{product.days ? `${product.days} Days` : "-"}</td>
                                                    <td className="p-4 font-mono font-bold text-[#ef3e36]">₹{product.price}</td>
                                                    <td className={`p-4 text-xs font-medium ${isDark ? "text-slate-400" : "text-slate-400"} truncate max-w-[200px]`}>{product.fileName}</td>
                                                    <td className="p-4 text-center space-x-4">
                                                        <button onClick={() => handleDeleteProduct(product._id)} className="text-[#ef3e36] font-bold text-xs uppercase tracking-wider hover:underline transition">Remove</button>
                                            ...
                                            {challengeProducts.length === 0 && (
                                                <tr>
                                                    <td colSpan="5" className={`p-8 text-center ${textSecondary} font-bold text-sm`}>No challenges available.</td>
                                                </tr>
                                            )}
                                        </tbody>
                                    </table>
                                    ...
                                    <tbody className={`divide-y ${isDark ? "divide-slate-700" : "divide-slate-100"}`}>
                                        {users.map(u => (
                                            <tr key={u._id} className={`text-sm ${isDark ? "hover:bg-slate-800" : "hover:bg-slate-50"}`}>
                                                ...
                                                <td className="p-5 text-right">
                                                    <button className="text-xs font-bold text-[#ef3e36] border border-[#ef3e36] hover:bg-[#ef3e36] hover:text-white px-4 py-2 rounded-lg transition uppercase tracking-widest shadow-sm">Manage Access</button>
                                                </td>
                                            </tr>
                                        ))}
                                        {users.length === 0 && (
